 ...
 */
export function assignRoles(memberIds: string[]): Record<string, Role> {
  const pool: Role[] = [
    Role.Werewolf,
    Role.Werewolf,
    Role.Thief,
    Role.Seer,
    ...Array(memberIds.length - 4).fill(Role.Villager),
  ];
 ...
 */
export function assignHeroes(members: string[]): Record<string, string> {
  const shuffled = [...Object.values(Hero)].sort(() => Math.random() - 0.5);
