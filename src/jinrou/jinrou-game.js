 ...
 * JSONファイルパス
 */
const filePath = "./json/jinrou-game.json";
...
export class JinrouGameMaster {
   ...
   * @returns
   */
  performRole(userId, targetId = undefined) {
    ...
    switch (user.roleCard.id) {
      case Role.Seer:
        return this.performSeer(userId, targetId);
      case Role.Werewolf:
        return this.performWerewolf(userId, targetId);
      case Role.Thief:
        return this.performThief(userId, targetId);
    }
  }
   ...
   */
  performSeer(userId, targetId = undefined) {
    if (!this.seerOptions.find((option) => option.id === userId)) {
      throw new Error("指定不可");
    }

    if (targetId) {
      return this.confirmationRoleIds(targetId);
    }

    const shuffledVillager = shuffle(this.villager.map((player) => player.id));
    const target = shuffledVillager.pop();
    return target;
  }
   ...
   */
  performWerewolf(userId, targetId = undefined) {
    ...
    return this.werewolves.map((player) => player.id);
  }
   ...
   * @returns {Role} 指定のプレイヤーの役職カードID
   */
  performThief(userId, targetId) {
    if (!this.thiefOptions.find((option) => option.id === userId)) {
      throw new Error("指定したプレイヤーは対象になりません。");
    }
  ...
  vote(targetId, userId = "") {
    ...
    if (target.effect.trunk) {
      const newTarget = this.players.find(
